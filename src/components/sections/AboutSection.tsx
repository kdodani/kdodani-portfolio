  ...
  return (
    <SectionFrame id="about" aria-labelledby="about-heading" viewportFocus>
      <div className="mx-auto w-full max-w-scene">
        ...
        <motion.div className="mt-6 grid gap-10 sm:mt-7 lg:mt-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-x-12 xl:gap-x-14">
          <motion.div
            className="order-2 min-w-0 lg:order-1"
            initial={fadeUpInitial(reduceMotion, 16)}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-56px" }}
            transition={{ duration: 0.6, delay: motionDelay(reduceMotion, 0.04), ease }}
          >
            <div className="max-w-editorial space-y-4 text-[15px] leading-[1.75] text-stone-600 sm:text-base sm:leading-[1.72]">
          ...
          <motion.div
            className="order-1 flex justify-center px-2 sm:px-4 lg:order-2 lg:justify-end lg:px-0"
          ...
          >
            <div className="w-full max-w-[10.5rem] shrink-0 sm:max-w-[11.5rem] lg:max-w-[12rem]">
              <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-stone-200/70 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
                <Image
                  src="/images/kd-headshot-color.png"
                  alt="Khushboo Dodani"
                  width={800}
                  height={800}
